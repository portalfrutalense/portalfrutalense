...
export async function GET(req: NextRequest) {
  ...
  const [{ data, error }, { data: perfis }] = await Promise.all([
    supabaseServer
      .from('demandas')
      .select('*, categoria:categorias_mapa(*), entidade:entidades(*), vinculos:demanda_entidades(id, status, resposta, respondido_em, resposta_ip, entidade:entidades(nome, cargo))')
